(function(){
  ...
  function init(){
    ...
    renderDays();
    log('JS OK v17 wod two columns · '+timeNow()+' · a pedir Supabase sem cachebuster REST...');
    loadState();
  ...
  function loadState(silent){
    ...
    try{
      ...
      xhr.onreadystatechange=function(){
        ...
        if(!silent || param('debug')==='1') appendLog('HTTP '+xhr.status+' · resposta '+String(xhr.responseText||'').length+' chars · v17');
  ...
  function renderCross(w,date){
    ...
    if(wod){
      var split=shouldSplitWodText(wod);
      html+='<div class="block wod'+(split?' is-two-column':'')+'"><div class="head"></div><div class="body">'+renderWodBody(wod,split)+'</div></div>';
    }
    els.sections.innerHTML=html;
  }
  function renderWodBody(text,split){
    if(!split) return '<pre>'+esc(text)+'</pre>';
    var parts=splitWodColumns(text);
    if(!parts || parts.length<2 || !parts[1]) return '<pre>'+esc(text)+'</pre>';
    return '<div class="two-col"><pre>'+esc(parts[0])+'</pre><pre>'+esc(parts[1])+'</pre></div>';
  }
  function shouldSplitWodText(text){
    ...
    var visible=0;
    for(var i=0;i<lines.length;i++){ if(String(lines[i]).replace(/\s/g,'')!=='') visible++; }
    return visible>=9 || cleaned.length>=230;
  }
  function splitWodColumns(text){
    var lines=clean(text).split('\n');
    if(lines.length<2) return [clean(text),''];
    var weights=[];
    var total=0;
    for(var i=0;i<lines.length;i++){
      var w=Math.max(1,Math.ceil(String(lines[i]||'').length/28));
      weights.push(w); total+=w;
    }
    var target=total/2, acc=0, split=Math.floor(lines.length/2);
    for(var j=0;j<weights.length;j++){ acc+=weights[j]; if(acc>=target){ split=j+1; break; } }
    var min=Math.max(1,Math.floor(lines.length*.25));
    var max=Math.min(lines.length-1,Math.ceil(lines.length*.78));
    if(split<min) split=min;
    if(split>max) split=max;
    var best=-1, bestDist=999;
    var start=Math.max(min,split-5), end=Math.min(max,split+5);
    for(var k=start;k<=end;k++){
      var line=String(lines[k]||'');
      var prev=String(lines[k-1]||'');
      var isBlank=line.replace(/\s/g,'')==='';
      var isHeader=/^(rx|scale|scaled|adaptado|adaptada|intermedio|interm[eé]dio|iniciante)\s*:/i.test(line.replace(/^\s+/,''));
      if(isBlank || isHeader){
        var dist=Math.abs(k-split)+(isHeader?0.25:0);
        if(dist<bestDist){ best=k; bestDist=dist; }
      }
      if(prev.replace(/\s/g,'')==='' && line.replace(/\s/g,'')!==''){
        var dist2=Math.abs(k-split)+0.5;
        if(dist2<bestDist){ best=k; bestDist=dist2; }
      }
    }
    if(best>0) split=best;
    var left=lines.slice(0,split).join('\n').replace(/\s+$/,'');
    var right=lines.slice(split).join('\n').replace(/^\s+/,'');
    return [left,right];
  }
