...
(() => {
  ...
  function applyVisualAssets() {
    ...
    const resolvedCrossBackground = crossBackground || (/training-bg-clean\.png(?:\?v=[A-Za-z0-9._-]+)?$/i.test(genericBackground) ? "assets/crosstraining-tv-background.png" : genericBackground || "assets/crosstraining-tv-background.png");
    const pairs = {
  ...
  function loadAndRender() {
    return __async(this, null, function* () {
      ...
      try {
        const loaded = yield loadTvState();
  ...
  function loadTvState() {
    return __async(this, null, function* () {
      var _a;
      if (shouldUseSupabase() && ((_a = window.supabase) == null ? void 0 : _a.createClient)) {
        ...
        const { data, error } = yield withTimeout(
          client.from(table).select("payload, updated_at").eq("id", id).maybeSingle(),
          12e3
        );
        if (error) throw error;
        if (data == null ? void 0 : data.payload) return { state: data.payload, updatedAt: data.updated_at, source: "online" };
      }
      const local = loadLocalState();
  ...
  function loadLocalState() {
    ...
    for (const key of keys) {
      try {
        ...
        const state = (parsed == null ? void 0 : parsed.state) && Array.isArray(parsed.state.workouts) ? parsed.state : parsed;
  ...
  function rememberTvSnapshot(state, updatedAt = "", source = "") {
    ...
    try {
      (_a = window.localStorage) == null ? void 0 : _a.setItem(TV_CACHE_STORAGE_KEY, JSON.stringify({ state, updatedAt, source }));
    } catch (e) {
    }
    const hyroxWorkouts = Array.isArray(state.hyroxWorkouts) ? state.hyroxWorkouts.filter((workout) => {
      const blocks = normalizeHyroxBlocks((workout == null ? void 0 : workout.blocks) || []).filter((block) => !isCoachNotesBlock(block));
      return (workout == null ? void 0 : workout.date) && blocks.length;
    }) : [];
    if (!hyroxWorkouts.length) return;
    try {
      (_b = window.localStorage) == null ? void 0 : _b.setItem(TV_HYROX_CACHE_KEY, JSON.stringify({ hyroxWorkouts, updatedAt }));
    } catch (e) {
    }
  }
  function loadCachedHyroxWorkouts() {
    var _a;
    try {
      const raw = (_a = window.localStorage) == null ? void 0 : _a.getItem(TV_HYROX_CACHE_KEY);
      if (!raw) return [];
      const parsed = JSON.parse(raw);
      const list = Array.isArray(parsed == null ? void 0 : parsed.hyroxWorkouts) ? parsed.hyroxWorkouts : [];
  ...
  function mergeHyroxWithCache(hyroxWorkouts = []) {
    ...
    hyroxWorkouts.forEach((workout) => {
      const publicBlocks = normalizeHyroxBlocks((workout == null ? void 0 : workout.blocks) || []).filter((block) => !isCoachNotesBlock(block));
      if ((workout == null ? void 0 : workout.date) && publicBlocks.length) byDate.set(workout.date, workout);
    });
    return Array.from(byDate.values()).sort((a, b) => String(a.date).localeCompare(String(b.date)));
  }
  function normalizePublicState(state) {
    const users = ((state == null ? void 0 : state.users) || []).map((user) => ({
    ...
    const normalizedHyroxWorkouts = Array.isArray(state == null ? void 0 : state.hyroxWorkouts) ? state.hyroxWorkouts.map((workout) => ({
      id: String((workout == null ? void 0 : workout.id) || (workout == null ? void 0 : workout.date) || ""),
      date: String((workout == null ? void 0 : workout.date) || ""),
      title: String((workout == null ? void 0 : workout.title) || "HYROX Session"),
      blocks: normalizeHyroxBlocks((workout == null ? void 0 : workout.blocks) || [])
    ...
    return {
      ...
      classes: Array.isArray(state == null ? void 0 : state.classes) ? state.classes.map((classEntry) => ({
  ...
  function renderTv() {
    ...
    tv.els.date.textContent = activeClass ? `${formatDateLong(date)} \xB7 ${activeClass.time}-${activeClass.endTime}` : formatDateLong(date);
    renderDayStrip(date);
    tv.els.lastUpdated.textContent = `\xDAltima atualiza\xE7\xE3o: ${formatDateTime(tv.updatedAt || (/* @__PURE__ */ new Date()).toISOString())}`;
    if (isHyrox) {
    ...
    if (!workout) {
      ...
      tv.els.workoutSections.innerHTML = `<article class="empty-tv-card">Ainda n\xE3o h\xE1 treino para ${escapeHtml(formatDateShort(date))}.</article>`;
    ...
    tv.els.workoutTags.innerHTML = renderTags([
      ...
      workout.movement,
      `For\xE7a: ${TV_SCORE_TYPES[workout.strengthScoreType || "load"] || "Carga"}`,
  ...
  function getDisplayContext(date) {
    ...
    const mode = forcedMode || normalizeClassType((activeClass == null ? void 0 : activeClass.classType) || "cross");
    return {
      workout,
  ...
  function renderHyroxTv(hyroxWorkout, activeClass, date) {
    ...
    tv.els.workoutSections.innerHTML = publicBlocks.length ? publicBlocks.map(renderHyroxBlock).join("") : `<article class="empty-tv-card">Ainda n\xE3o h\xE1 programa\xE7\xE3o HYROX p\xFAblica para ${escapeHtml(formatDateShort(date))}.</article>`;
  }
  ...
  function renderHyroxBlock(block) {
    ...
    <article class="block-card hyrox-block hyrox-${escapeAttr(normalizeHyroxBlockType(block.type))}${densityClass}">
      ...
      <div class="block-body hyrox-block-body"><pre>${escapeHtml(body || "Sem conte\xFAdo programado.")}</pre></div>
    </article>
  `;
  }
  ...
  function fitHyroxLayout() {
    ...
    const setScale = (scale2) => {
      sections.style.setProperty("--hyrox-body-font", `${Math.max(13, Math.round(baseBodyFont * scale2))}px`);
      sections.style.setProperty("--hyrox-title-font", `${Math.max(17, Math.round(baseTitleFont * scale2))}px`);
      sections.style.setProperty("--hyrox-label-font", `${Math.max(9, Math.round(baseLabelFont * scale2))}px`);
      sections.style.setProperty("--hyrox-duration-font", `${Math.max(10, Math.round(baseDurationFont * scale2))}px`);
      sections.style.setProperty("--hyrox-body-pad", `${Math.max(7, Math.round(18 * scale2))}px`);
      sections.style.setProperty("--hyrox-head-pad", `${Math.max(7, Math.round(16 * scale2))}px`);
  ...
  function renderLiveClassPin(workout) {
    ...
    const selectedDate = (workout == null ? void 0 : workout.date) || getSelectedDate();
  ...
  function getActiveClassForTv(date, now = /* @__PURE__ */ new Date()) {
  ...
  function getClassPinToShowForTv(date, now = /* @__PURE__ */ new Date()) {
    const classes = getClassesForDate(date).sort((a, b) => String(a.time || "").localeCompare(String(b.time || "")));
    if (!classes.length) return null;
    return classes.find((classEntry) => !classEntry.ended && isClassPinActive(classEntry, now)) || null;
  }
  function getClassPinStatusForTv(classEntry, now = /* @__PURE__ */ new Date()) {
    ...
    if (now < opensAt) return { label: `V\xE1lido \xE0s ${formatTimeOnly(opensAt)}`, tone: "is-waiting" };
    if (now <= expiresAt) return { label: `V\xE1lido at\xE9 ${formatTimeOnly(expiresAt)}`, tone: "" };
    return { label: `Expirado \xE0s ${formatTimeOnly(expiresAt)}`, tone: "is-expired" };
  }
  ...
  function getClassesForDate(date) {
    var _a;
    return (((_a = tv.state) == null ? void 0 : _a.classes) || []).filter((classEntry) => classEntry.date === date && classEntry.time && classEntry.endTime);
  }
  ...
  function isClassPinActive(classEntry, now = /* @__PURE__ */ new Date()) {
  ...
  function renderBlock(kind, title, text) {
    ...
    const twoColumn = kind === "strength" && shouldSplitStrengthText(cleaned) || kind === "wod" && shouldSplitWodText(cleaned);
  ...
  function hasProgrammedWarmup(text) {
    ...
    const normalized = cleaned.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[–—]/g, "-").replace(/\s+/g, " ").trim();
  ...
  function renderTags(tags) {
    return tags.map((tag) => String(tag || "").trim()).filter(Boolean).slice(0, 4).map((tag) => `<span class="tv-tag">${escapeHtml(tag)}</span>`).join("");
  }
  function renderTopResults(workout) {
    if (!workout) return emptySmall("Sem WOD selecionado.");
    const rows = getResultsForWorkout(workout).map((result) => __spreadProps(__spreadValues({}, result), { __tvWodScore: getTvWodScore(result, workout) })).filter((result) => result.__tvWodScore).sort((a, b) => compareResults(a, b, workout)).slice(0, 3);
    if (!rows.length) return emptySmall("Ainda sem resultados de WOD.");
    return rows.map((result, index) => {
      var _a;
      const value = result.__tvWodScore || "--";
      const name = isTeamResult(result) ? formatTeamResultName(result, { compact: true }) : ((_a = getUser(result.userId)) == null ? void 0 : _a.name) || "Atleta";
  ...
  function getResultTeamUserIds(result = {}) {
    const ids = Array.isArray(result == null ? void 0 : result.teamUserIds) ? result.teamUserIds : [];
    const fallback = (result == null ? void 0 : result.userId) ? [result.userId] : [];
  ...
  function formatTeamResultName(result, options = {}) {
    var _a;
    const names = getResultTeamUserIds(result).map((id) => {
      var _a2;
      return (_a2 = getUser(id)) == null ? void 0 : _a2.name;
    }).filter(Boolean);
    if (!names.length) return ((_a = getUser(result == null ? void 0 : result.userId)) == null ? void 0 : _a.name) || "Team";
  ...
  function getTvWodScore(result, workout) {
    const metcon = String((result == null ? void 0 : result.metconScore) || "").trim();
    if (metcon) return isDnfScore(metcon) ? "DNF" : metcon;
    const generic = String((result == null ? void 0 : result.score) || "").trim();
  ...
  function extractMetconScoreFromRelatedFeed(result, workout) {
    ...
    const feedItem = (tv.state.feed || []).filter((item) => {
    ...
    }).sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || ""))).find((item) => extractMetconScoreFromText(item.text || "", workout.scoreType));
  ...
  function looksLikeStrengthOnlyResultText(text) {
    const normalized = String(text || "").normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase().replace(/[–—]/g, "-").replace(/\s+/g, " ").trim();
  ...
  function renderActivityFeed(workout) {
    ...
    const latestItems = (tv.state.feed || []).filter((item) => !workoutIds || !item.workoutId || workoutIds.has(item.workoutId) || String(item.workoutId).includes(workout.date)).sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")));
    ...
    return items.map((item) => {
      ...
      return `
        <article class="activity-row">
          <strong>${escapeHtml((user == null ? void 0 : user.name) || "Atleta")}</strong>
          <span>${escapeHtml(type)} \xB7 ${escapeHtml(formatDateTime(item.createdAt))}</span>
  ...
  function formatActivityTextForTv(text) {
    const cleaned = String(text || "").replace(/^registou\s+/i, "").replace(/\s+/g, " ").trim();
  ...
  function renderCommentFeed(workout) {
    ...
    const latest = comments.filter((comment) => String(comment.text || "").trim()).sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || ""))).slice(0, 3);
    if (!latest.length) return emptySmall("Sem coment\xE1rios recentes.");
    return latest.map((comment) => {
      const author = getUser(comment.userId);
      return `
        <article class="activity-row">
          <strong>${escapeHtml((author == null ? void 0 : author.name) || "Coach")}</strong>
  ...
  function renderDayStrip(selectedDate) {
    ...
    const today = isoDate(/* @__PURE__ */ new Date());
    tv.els.dayStrip.innerHTML = days.map((date) => {
      const active = date === selectedDate;
      const isToday = date === today;
      const day = /* @__PURE__ */ new Date(`${date}T12:00:00`);
      ...
      const classes = ["tv-day-button", active ? "is-active" : "", isToday ? "is-today" : ""].filter(Boolean).join(" ");
      return `
  ...
  function getWeekDates(selectedDate) {
    const selected = /* @__PURE__ */ new Date(`${selectedDate}T12:00:00`);
  ...
  function normalizeHyroxBlocks(blocks = []) {
    return (Array.isArray(blocks) ? blocks : []).map((block, index) => ({
      id: String((block == null ? void 0 : block.id) || `hb-${index}`),
  ...
  function compareResults(a, b, workout) {
    const type = (workout == null ? void 0 : workout.scoreType) || "time";
  ...
  function renderError(error) {
    ...
    tv.els.workoutName.classList.remove("is-hidden");
    tv.els.workoutTags.innerHTML = "";
    tv.els.workoutSections.innerHTML = `<article class="empty-tv-card">${escapeHtml((error == null ? void 0 : error.message) || "Erro desconhecido.")}</article>`;
